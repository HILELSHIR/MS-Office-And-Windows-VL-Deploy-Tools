...
title Online Installation Tool
...
if not defined xTmp (
	if not exist BuildInfo.ini (
		...
		echo Supprted System Bit 	:: [32, 64, Multi] [Fallback - System Arch]
		...
		echo.
		echo "%~n0" 			   - Office $$$ 2021 - [System Arch] - [System Language]
		echo "%~n0" 2016 64 		   - Office $$$ 2016 - X64 - [System Language]
		echo "%~n0" 2013 multi hebrew    - Office $$$ 2013 - X32-X64 - Hebrew
		echo.
		echo "%~n0" 1 		   - Lite - Office 2021 - [System Arch] - [System Language]
		echo "%~n0" 2 2016 64 	   - Full - Office 2016 - X64 - [System Language]
		echo "%~n0" 3 2013 multi hebrew  - VIP  - Office 2013 - X32-X64 - Hebrew
		echo "%~n0" 4 Russian  	   - 365  - Russian
		echo.
...
if /i '!ProductYear!' EQU '2016' (
	...
	>>%cnfg% echo ^<Add OfficeClientEdition^=^"!xBit!^" Channel^=^"Current^" Version^=^"!version!^"^>
	...
	echo Build Configuration File for Office !ProductYear! x!xBit! !langCd!
	>%cnfg%  echo ^<Configuration^>
	>>%cnfg% echo ^<Add OfficeClientEdition^=^"!xBit!^" Channel^=^"PerpetualVL!ProductYear!^" Version^=^"!version!^"^>
...
if /i '!ProductYear!' EQU '2013' (
	if not exist !OfficeClickToRun! (
		set clr_path=%ProgramFiles%\Microsoft Office 15\ClientX!xBit!
		1>nul 2>&1 rd /s /q "!clr_path!"
		1>nul 2>&1 md "!clr_path!"
		1>nul 2>&1 tools\wget --quiet --tries=3 --retry-connrefused --output-document="!clr_path!\i!xBit!!langID!.cab" "officecdn.microsoft.com.edgesuite.net/pr/!ProductId!/Office/Data/!version!/i!xBit!!langID!.cab"
		if exist "!clr_path!\i!xBit!!langID!.cab" 1>nul 2>&1 expand -f:*.* "!clr_path!\i!xBit!!langID!.cab" "!clr_path!"
		1>nul 2>&1 del /q "!clr_path!\i!xBit!!langID!.cab"
	)
)

if /i '!ProductYear!' NEQ '2013' (
	if not exist !OfficeClickToRun! (
		set clr_path=%mShared%\ClickToRun
		1>nul 2>&1 rd /s /q "!clr_path!"
		1>nul 2>&1 md "!clr_path!"
		1>nul 2>&1 tools\wget --quiet --tries=3 --retry-connrefused --output-document="!clr_path!\i!xBit!0.cab" "officecdn.microsoft.com.edgesuite.net/pr/!ProductId!/Office/Data/!version!/i!xBit!0.cab"
		if exist "!clr_path!\i!xBit!0.cab" 1>nul 2>&1 expand -f:*.* "!clr_path!\i!xBit!0.cab" "!clr_path!"
		1>nul 2>&1 del /q "!clr_path!\i!xBit!0.cab"
	)
)
...
>"%temp%\tmp" call :Language_List
for /f "tokens=1,2,3 delims=*" %%g in ('type "%temp%\tmp"') do if exist "office\data\!version!\i64%%h.cab" set lLngID=%%h

for %%g in (2013, 2016, 2019, 2021) do (
	( echo %fCD% | find /i "%%g">nul && (set "lYear=%%g"&set "userSelected=***") ) || ( vol | find /i "%%g">nul && (set "lYear=%%g"&set "userSelected=***") )
)
goto :eof
